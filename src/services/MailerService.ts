...
class MailerService {
    ...
    public async sendInitialTestEmail(): Promise<TestEmailResult> {
        ...
        try {
            ...
            // Aguarda o resultado do email de teste
            const testResult = await this.emailService.waitForTestEmailResult(requestUuid);
            if (testResult.success) {
                logger.info(`Email de teste enviado com sucesso. mailId: ${testResult.mailId}`);
                this.unblockMailer();
                return { success: true, mailId: testResult.mailId };
            } else {
                ...
                return { success: false, mailId: testResult.mailId };
            }
        } catch (error: any) {
