...
class EmailController {
  ...
  async sendNormal(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      // **1. Remover Duplicatas da `emailList`:**
      ...
      for (const emailData of uniqueEmailList) {
        ...
        const result = await emailService.sendEmail(emailPayload, uuid, emailQueue.queueIds); // Passa emailQueue.queueIds para o EmailService
